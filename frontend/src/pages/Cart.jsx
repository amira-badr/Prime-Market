    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {cart.map((item) => (
          <div key={item.id} className="flex items-center gap-4 bg-[#1c164a]/60 backdrop-blur-md p-4 rounded-[1.5rem] border border-white/5">
            ...
            <div className="flex-grow">
              <h4 className="font-black text-sm line-clamp-1">{item.title}</h4>
      ...
      <div className="bg-[#130d33] border border-white/5 p-6 rounded-[2rem] h-fit sticky top-24">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between font-black text-base"><span>الإجمالي النهائي:</span><span className="text-cyan-400">{cartTotal} EGP</span></div>
        </div>
